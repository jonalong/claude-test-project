...
function AppBar({ showTooltip, onToggleTooltip, onLogoClick }) {
...
export default function App() {
  ...
  // ── CTA 버튼 키보드 추적 (input/tone 단계만) ─────────────────────
  // 핵심 수정: kb = window.innerHeight - vv.height
  // vv.offsetTop은 페이지 스크롤 양이므로 키보드 높이와 무관 → 빼면 안됨
  useEffect(() => {
    ...
    function update() {
      ...
      // 키보드 높이 = 전체 화면 - 보이는 영역. offsetTop 제외
    ...
    // focusin 시점부터 rAF 폴링: 키보드 애니메이션(~300ms) 커버
    // iOS/카카오/인스타 인앱 브라우저에서 resize 이벤트 지연 대응
    let rafId = null
    function startPolling() {
      ...
      ;(function poll() {
        update()
        if (Date.now() < end) rafId = requestAnimationFrame(poll)
      })()
    }
  ...
  if (step === 'result' && result) {
    ...
    return (
      <div className="min-h-[100dvh] bg-white flex flex-col max-w-[460px] mx-auto">
        ...
        {/* 복사 토스트 */}
        <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#121212] text-white text-[14px] font-medium px-4 py-3 rounded-lg whitespace-nowrap pointer-events-none transition-opacity duration-300 z-20 ${copied ? 'opacity-100' : 'opacity-0'}`}>
          ✓　클립보드에 복사되었습니다
        </div>
      </div>
    )
